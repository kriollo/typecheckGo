...
func NewGlobalEnvironmentWithLibs(libs []string) *GlobalEnvironment {
	...
	// DOM and Browser APIs are now loaded from lib.dom.d.ts
	// These hardcoded definitions are kept as fallback for projects without TypeScript libs
	if hasDOM {
		...
		/*
			...
			windowType := NewObjectType("Window", map[string]*Type{
				...
				"open":           NewFunctionType([]*Type{String, String}, Any),
				"close":          NewFunctionType([]*Type{}, Void),
				"localStorage":   Any,
				"sessionStorage": Any,
				"innerWidth":     Number,
				"innerHeight":    Number,
