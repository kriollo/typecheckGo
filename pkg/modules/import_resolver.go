...
func (ir *ImportResolver) ResolveImport(importDecl *ast.ImportDeclaration) (map[string]*symbols.Symbol, error) {
	...
	if namespaceSpecifier != nil {
		// Build an ObjectType with all exports as properties
		exportProperties := make(map[string]*types.Type)

		for exportName, exportInfo := range resolvedModule.Exports {
			// Resolve re-export chains to get the original type
			var finalExport *ExportInfo
			if exportInfo.IsReExport && exportInfo.SourceModule != "" {
				finalExport = ir.resolveReExportChain(exportInfo, resolvedModule.AbsolutePath)
				if finalExport == nil {
					finalExport = exportInfo
				}
			} else {
				finalExport = exportInfo
			}

			// Determine the type based on the exported node
			var exportType *types.Type
			if finalExport.ResolvedType != nil {
				exportType = finalExport.ResolvedType
			} else if finalExport.Node != nil {
				// Infer type from the node
				exportType = ir.inferTypeFromNode(finalExport.Node)
			} else {
				exportType = types.Any
			}

			exportProperties[exportName] = exportType
		}

		// Create namespace type as an ObjectType with exported members
		namespaceType := types.NewObjectType("typeof "+namespaceSpecifier.Local.Name, exportProperties)
...
// inferTypeFromNode infers a type from an AST node
func (ir *ImportResolver) inferTypeFromNode(node ast.Node) *types.Type {
	if node == nil {
		return types.Any
	}
	...
	switch n := node.(type) {
	case *ast.VariableDeclaration:
		// For variable declarations, infer from first declarator's initializer
		if len(n.Decls) > 0 && n.Decls[0] != nil {
			decl := n.Decls[0]
			if decl.Init != nil {
				return ir.inferTypeFromExpression(decl.Init)
			}
		}
		return types.Any

	case *ast.FunctionDeclaration:
		// For functions, return a FunctionType
		return &types.Type{Kind: types.FunctionType, IsFunction: true}

	case *ast.ClassDeclaration:
		// For classes, return an ObjectType with the class name
		if n.ID != nil {
			return &types.Type{Kind: types.ObjectType, Name: n.ID.Name}
		}
		return types.Any
...
// inferTypeFromExpression infers a type from an expression for simple cases
func (ir *ImportResolver) inferTypeFromExpression(expr ast.Expression) *types.Type {
	if expr == nil {
		return types.Any
	}
	...
	case *ast.Literal:
		...
		switch e.Value.(type) {
		case string:
			return types.Number // Number literals are often stored as strings in the parser
		case int, int64, float64:
			return types.Number
		case bool:
			return types.Boolean
		default:
			// Check Raw for actual value type
			if e.Raw != "" && len(e.Raw) > 0 {
				first := e.Raw[0]
	...
	case *ast.ArrayExpression:
		return &types.Type{Kind: types.ArrayType, ElementType: types.Any}

	case *ast.ObjectExpression:
		return &types.Type{Kind: types.ObjectType}
