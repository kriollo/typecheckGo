...
const appCustomPedido = new Vue({
    ...
    setup() {
        const customPedido = Vue.ref({});

        Vue.onMounted(() => {
            const cp = $dom('#customPedido');
            if (cp instanceof HTMLElement) {
                customPedido.value = JSON.parse(cp.getAttribute('data-value'));
            }
        });
    ...
    methods: {
        ...
        GotoSolicitud: function (id) {
            location.href = `/bodega/custom_pedido_ingreso_${this.customPedido.id}/${id}`;
        },
        ...
        ClonePedido: function (id) {
            location.href = `/bodega/custom_pedido_ingreso_${this.customPedido.id}/clonepedido${id}`;
        },
        ...
        pasarella(event) {
            const actions = {
                ...
                GotoSolicitud: () => {
                    location.href = `/bodega/custom_pedido_ingreso_${this.customPedido.id}/${event.id}`;
                },
...
document.addEventListener('click', function (event) {
    pasarella(appCustomPedido, event);
});
