...
async function obtener_select_item(self, tabla, selec, inp) {
    ...
    const opt = elem.querySelector(`option[value='${val}']`);
    let match = '0';
    if (opt != null) match = opt.getAttribute('data-value2');
...
const _appCustomPedido = new Vue({
    el: '#content',
    delimiters: ['${', '}'],
    data: function () {
        return {
    ...
    mounted: async function () {
        const cp = $dom('#customPedido');
        if (cp instanceof HTMLElement) {
            this.pedidoSeleccionado = JSON.parse(cp.getAttribute('data-value'));
        }
        ...
        const getFamiliasByCodigoHabilitaPedido = async () => {
            const response = await versaFetch({
                url: '/api/custom/getFamiliasByCodigoHabilitaPedido',
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                data: JSON.stringify({
                    custom_pedido: this.pedidoSeleccionado.id,
                }),
            });
            return response;
        };
    ...
    methods: {
        load_codigos: async function (id) {
            ...
            const response = await versaFetch({
                url: '/api/custom/getProductosByCodigoHabilitaPedido',
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                data: JSON.stringify({
                    custom_pedido: this.pedidoSeleccionado.id,
                    showStock: this.pedidoSeleccionado.show_stock,
                    showWhithStock: this.pedidoSeleccionado.show_with_stock,
                    Cod_familia1: id,
        ...
        save_doc_solicitud: async function (id) {
            ...
            if (error == false) {
                ...
                if (json.success == 1) {
                    ...
                    localStorage.items = [];
                    setTimeout(() => {
                        location.href = `/bodega/custom_pedido_ingreso_${this.pedidoSeleccionado.id}`;
                    }, 1000);
                } else {
