...
const _appTipoCodigo = new Vue({
    ...
    methods: {
        ...
        edit_reg_master: async function (response) {
            ...
            let api = '';
            if (mode == 'new') api = 'newTipoCodigo';
            else api = 'editTipoCodigo';
            ...
            if (json.success == 1) {
                show_toast('Success', json.title, json.message, 'success');
                ...
                show_toast('Alerta', json.title, json.message, 'warning');
            }
        },
...
window.addEventListener('mouseup', function (e) {
    ...
    if (testData.style[0] == undefined && flat == FALSE) {
        _appTipoCodigo.showModal = false;
    }
});

document.addEventListener('click', function (event) {
    pasarella(_appTipoCodigo, event);
});
