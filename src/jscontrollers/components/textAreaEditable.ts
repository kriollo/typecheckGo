...
// TODO: cambiar input por textarea
...
const textAreaEditable = {
    ...
    setup(props) {
        const dataProps = Vue.computed(() => props.data);
        const idProps = Vue.computed(() => props.id);
        const estado_panel = Vue.computed(() => props.estado_panel);
        const newData = Vue.ref(JSON.parse(JSON.stringify(dataProps.value)));
        const field = Vue.computed(() => props.field);

        const showCancel = Vue.computed(() => props.showCancelButton);
...
Vue.component('textAreaEditable', textAreaEditable);
