import { read, utils, writeFile } from 'P@/vendor/plugins/xlsx/xlsx.full.min.esm.js';
 ...
 */
export const exportToExcel = async (data: {}[], sheetName: string = 'Sheet1'): Promise<void> => {
 ...
 * @returns {Promise<void>} - A promise that resolves when the XLSX file is created.
 */
export const createXlsxFromJson = async (data: {}[], sheetName: string = 'Sheet1'): Promise<void> => {
    const wb = utils.book_new();
    const ws = utils.json_to_sheet(data);
    utils.book_append_sheet(wb, ws, sheetName);
    writeFile(wb, `${sheetName}.xlsx`);
...
export const useXlsx = () => {
    const exportToExcel = async (data, sheetName: string = 'Sheet1'): Promise<void> => {
        const wb = utils.book_new();
        const ws = utils.json_to_sheet(data);
        utils.book_append_sheet(wb, ws, sheetName);
        writeFile(wb, `${sheetName}.xlsx`);
